...
function ConstellationBG() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < POINTS; i++) {
      pts.push({ x: rand(canvas.width), y: rand(canvas.height), vx: (Math.random() - 0.5) * 0.2 * DPR, vy: (Math.random() - 0.5) * 0.2 * DPR });
    }
    ...
    const draw = () => {
      ...
      for (let i = 0; i < pts.length; i++) {
        for (let j = i + 1; j < pts.length; j++) {
          ...
          const maxD2 = (180 * DPR) ** 2;
          if (d2 < maxD2) {
            const alpha = 0.14 * (1 - d2 / maxD2);
            ctx.beginPath();
...
function CodeOverlay() {
  const lines = Array.from({ length: 20 }, (_, i) =>
    `if(x>${i}){return hash(${i}^seed)<<2;} // pseudo cyber code ${i}`
  );
  return (
    <div className="pointer-events-none absolute inset-0 opacity-20 select-none mix-blend-screen">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_10%_10%,rgba(32,140,255,0.12),transparent_40%),radial-gradient(circle_at_90%_20%,rgba(132,56,255,0.10),transparent_35%),radial-gradient(circle_at_50%_100%,rgba(0,255,204,0.10),transparent_45%)]" />
      <pre className="text-[10px] leading-4 text-cyan-200/40 font-mono whitespace-pre break-words p-6 h-full overflow-hidden">
        {lines.join("\n\n").repeat(20)}
      </pre>
    </div>
  );
...
// === H shield logo (SVG) ===
function HenryShield({ className = "w-44 h-44" }: { className?: string }) {
  return (
    <svg viewBox="0 0 220 260" className={className}>
      <defs>
        <linearGradient id="g1" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor="#42e9ff" />
          <stop offset="100%" stopColor="#5c7cff" />
        </linearGradient>
      ...
      <path d="M110 10 L190 40 Q200 120 190 190 L110 240 L30 190 Q20 120 30 40 Z" fill="#0e1726" stroke="url(#g1)" strokeWidth="6" filter="url(#glow)"/>
      <g>
        <rect x="62" y="76" width="96" height="112" rx="12" fill="#1b2a44" stroke="#7cc8ff" strokeWidth="3"/>
        <rect x="72" y="86" width="76" height="92" rx="8" fill="#0d1b2b" />
        <path d="M88 172 V92 H102 V124 H118 V92 H132 V172 H118 V138 H102 V172 Z" fill="url(#g1)" />
      </g>
      {/* decorative nodes */}
      <circle cx="60" cy="60" r="5" fill="#21f3d6" />
      <circle cx="160" cy="60" r="5" fill="#ffd83b" />
      <circle cx="110" cy="45" r="4" fill="#68a0ff" />
    </svg>
  );
}
...
function LivePill() {
  return (
    <div className="absolute right-6 top-6 flex items-center gap-2">
      <span className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-pulse shadow-[0_0_12px_3px_rgba(16,185,129,0.65)]"></span>
      <span className="uppercase tracking-widest text-xs font-semibold text-emerald-300 drop-shadow">LIVE</span>
    </div>
  );
}

export default function HenryHero() {
  return (
    <div className="min-h-[680px] w-full bg-[#0b0f14] text-white relative overflow-hidden">
      {/* layered background */}
      <div className="absolute inset-0 bg-[radial-gradient(1200px_600px_at_10%_20%,rgba(0,255,200,0.10),transparent),radial-gradient(1000px_600px_at_90%_10%,rgba(66,140,255,0.10),transparent)]" />
      <ConstellationBG />
      <CodeOverlay />
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 pt-16 pb-10">
        ...
        <div className="flex flex-col md:flex-row items-center gap-8">
          <div className="shrink-0">
            <div className="relative">
              <HenryShield className="w-40 h-40 md:w-48 md:h-48" />
              {/* particle halo */}
              <div className="absolute -inset-6 rounded-full blur-2xl bg-cyan-400/10" />
            </div>
          </div>

          <div className="flex-1">
            <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight leading-none">HENRY</h1>
            <p className="mt-3 text-sm md:text-base text-cyan-100/90 max-w-3xl">
              <span className="font-semibold">Heroes Early Notification & Response Yesterday</span> — Powered by the Henry Protocol Predictive Algorithm.
            </p>

            <h2 className="mt-6 inline-block text-2xl md:text-3xl font-bold tracking-wide text-[#8ab6ff] relative">
              <span className="bg-gradient-to-r from-[#7cc8ff] to-[#4ea3ff] bg-clip-text text-transparent">AI‑Powered Veteran Crisis Prevention</span>
              <span className="block h-[3px] w-full mt-2 bg-gradient-to-r from-[#7cc8ff] via-[#6df3ff] to-transparent rounded-full" />
            </h2>

            <p className="mt-4 text-sm md:text-base text-slate-200/90">
              30–60 day predictive analysis • <span className="font-semibold text-white">94% F2 accuracy</span>
            </p>
          </div>
        </div>
        ...
        <div className="mt-10 grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { title: "94% F2 Accuracy", desc: "Model precision tuned for early alerts." },
            { title: "24/7 Monitoring & Notification", desc: "Always‑on signals and smart routing." },
            { title: "TERA Exposure Summaries", desc: "Concise toxic exposure context at a glance." },
            { title: "Detailed Records Insights", desc: "Actionable patterns across service history." },
          ].map((item, i) => (
            <div key={i} className="relative p-5 rounded-2xl bg-white/5 backdrop-blur border border-white/10 shadow-[0_0_0_1px_rgba(255,255,255,0.04)_inset]">
              <div className="absolute -top-px -left-px w-24 h-[2px] bg-gradient-to-r from-cyan-400 via-sky-400 to-transparent" />
              <div className="absolute -bottom-px -right-px w-24 h-[2px] bg-gradient-to-l from-fuchsia-500 via-purple-500 to-transparent" />
              <h3 className="text-lg font-semibold tracking-wide text-white drop-shadow-sm">{item.title}</h3>
              <p className="mt-2 text-sm text-slate-200/80 leading-relaxed">{item.desc}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-12 text-center">
          <p className="text-slate-300/90 text-sm">
            In memory of <span className="font-semibold">LCpl Christopher J. Henry, USMC</span>
          </p>
        </div>
      </div>
    </div>
...
### 🎯 **METHOD 1: One-Click Vercel Deploy** (Recommended)
  ...
  <table>
    ...
    <tr>
      <td align="center">
        ...
        <sub>Conservative estimate, crisis prevention</sub>
      </td>
      <td align="center">
        <h3>6</h3>
...
**DOCUMENT CATALOG**: *Every investigation needs proper documentation. These files contain the complete forensic record of how this system was built, deployed, and operated.*
